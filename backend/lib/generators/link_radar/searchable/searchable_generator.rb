...
module LinkRadar
  module Generators
    # Generator for creating searchable model configuration with pg_search
    #
    # This generator creates all necessary files for making a model searchable:
    # - SearchContent class defining search configuration
    # - SearchContent spec with smoke tests
    # - Migration for search_projection column (if --project)
    # - Rake task for rebuilding projections (if --project)
    # - Model injection (unless --skip-model-injection)
    #
    # @example Simple searchable model
    #   rails generate link_radar:searchable tag name description
    #
    # @example Complex model with projection
    #   rails generate link_radar:searchable link title content_text note --project
    #
    # @example Skip model modification
    #   rails generate link_radar:searchable link --skip-model-injection
    class SearchableGenerator < Rails::Generators::NamedBase
      ...
      def create_search_content_spec
      ...
      def self.next_migration_number(dirname)
        # Rails 8.1: always use timestamped migrations
